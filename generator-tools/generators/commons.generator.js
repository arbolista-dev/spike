...
function fnGenerateRoutes(name, templateDir, dest, routesDir) {
  const routes = getFolders(routesDir);
  ...
  for (let i = 0; i <= routes.length; i++) {
    if (routes[i] != undefined && routes[i].trim().length != 0) {
      const newRoute = generateHelper.data(routes[i]);
      newRoute.originalName = routes[i];
      routesClassNames.push(newRoute);
    }
  }
  const data = generateHelper.data(name);
...
function getFolders(sourceDir) {
  return fs.readdirSync(sourceDir).filter(file =>
     fs.statSync(path.join(sourceDir, file)).isDirectory() && file != 'missing' && file != 'index'
  );
}
